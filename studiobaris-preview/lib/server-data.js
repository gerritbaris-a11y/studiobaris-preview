...
export async function getTeamLogin() {
  const data = await rest("app_users?select=id,naam,rol,password_hash,vergoeding_model,maand_vast_bedrag&order=rol.asc,naam.asc");
  return (Array.isArray(data) ? data : []).map((u) => ({
    id: u.id, naam: u.naam, rol: u.rol, gezet: !!u.password_hash,
    vergoeding_model: u.vergoeding_model || "50pct",
    maand_vast_bedrag: u.maand_vast_bedrag,
  }));
}
