...
// `team`: {naam, rol}[] van getTeam() — een keuzelijst i.p.v. vrij tekstveld
// voorkomt dat een tikfout de omzet-/portefeuillekoppeling stilletjes breekt.
export default function AkkoordForm({ team = [] }) {
  ...
  return (
    <main style={wrap}>
      ...
      <div style={cardBox}>
        <label style={label}>Wie krijgt deze verkoop op zijn naam?</label>
        <select style={input} value={verzamelaar} onChange={(e) => setVerzamelaar(e.target.value)}>
          <option value="">— kies —</option>
          {team.map((t) => (
            <option key={t.naam} value={t.naam}>{t.naam}</option>
          ))}
        </select>
      </div>
